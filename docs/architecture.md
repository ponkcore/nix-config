...
theme/                             ── Nix-owned theme framework
├── default.nix      single active theme export (palette + structural defaults)
...
| `caelestia-shell` | forked Caelestia shell (Quickshell-based, `ponkcore/shell`) | follows |
| `caelestia-cli` | forked Caelestia CLI (`ponkcore/cli`) | follows |
...
Codex CLI is pinned to OpenAI's official x86_64 Linux prebuilt release in
`home/codex.nix`; Nix verifies the upstream checksum and installs the static
musl package without a local Rust build. A launcher sources `/run/agenix/tokens`,
obtains a GitHub token with `gh auth token`, and exports credentials only to the
Codex process. The shared MCP stack
is a low-precedence system layer at `/etc/codex/config.toml`, generated by
`modules/nixos/codex.nix`; mutable model selection, reasoning effort, project
trust, authentication, and sessions remain under `~/.codex`. The stack is
Context7, GitHub MCP, mcp-nixos, OmniRoute, Repomix, and Semgrep. Global agent
instructions are deployed to `~/.codex/AGENTS.md` by `home/agent-rules.nix`.
...
| Agent global instructions | `home/agent-rules.nix` (deploys `home/agent-instructions/AGENTS.md` to opencode/omp/agy/codex paths) |
| Codex CLI package + credentials | `home/codex.nix` |
| Codex shared MCP layer | `modules/nixos/codex.nix` (`/etc/codex/config.toml`) |

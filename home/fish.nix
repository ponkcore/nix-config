  ...
  programs.fish = {
    ...
    functions = {
      ...
      ts-on = ''
        ...
        # (gVisor stack) TUN; it conflicts with tailscaled on DNS
        # hijack + default-route ownership. TUN devices report state
        # UNKNOWN in `ip -br` (no link layer), so we check for
        # interface existence via `ip link show` instead of matching
        # the state column.
        set tun_found 0
        for iface in Mihomo Meta
        ...
        if test $tun_found -eq 1
          echo "ERROR: a proxy TUN (Clash Verge) appears to be up. Disable it first." >&2
          echo "       Hint: clash-verge-toggle (GUI), then re-run ts-on." >&2
          return 1
        end
